...
class SimpleCalculatorTest extends TestCase
{
    ...
    {
        $result = (new SimpleCalculator())->calculate(1000, ['a' => 0.5, 'b' => 0.3, 'c' => 0.2]);
    ...
    {
        $result = (new SimpleCalculator())->calculate(999, ['a' => 0.5, 'b' => 0.3, 'c' => 0.2]);
        $this->assertEquals(997, array_sum($result['distribution']));
        $this->assertEquals(2.0, $result['total']);
        $this->assertEquals(2.0, array_sum($result['roundings']));
